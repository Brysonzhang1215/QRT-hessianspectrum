"""Provide an HHL implementation compatible with mindquantum.algorithm.library.hhl.

This module implements the HHL routine using MindQuantum primitives and a
minimal interface so it can be imported as ``from hhl_provider import hhl``.
"""
...
def _apply_controlled_rotations(
    ...
    for k, lam in enumerate(lambdas):
        ...
        bitstring = format(k, f"0{num_phase}b")
        flipped: list[int] = []
        for bit, qubit in zip(bitstring, phase_qubits):
            if bit == "0":
                circ += X.on(qubit)
...
def hhl(matrix: np.ndarray, b: np.ndarray) -> tuple[Circuit, Callable, Callable]:
    """Build an HHL circuit along with state prep and decoder callables."""
    ...
    # HHL requires (at minimum) a Hermitian matrix; our rotation schedule further
    # assumes positive eigenvalues (SPD). If the input is not numerically
    # Hermitian, eigenvalues may pick up small imaginary components and trigger
    # ComplexWarning when coerced to float.
    herm_tol = 1e-10
    ...
    # For Hermitian matrices, eigvalsh is real and more stable than eigvals.
    ...
    phase_qubits = max(3, min(5, input_qubits + 1))
...
def _run_demo() -> None:
    """Run a simple 2x2 linear system through the HHL circuit."""
